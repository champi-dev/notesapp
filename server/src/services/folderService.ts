...
export class FolderService {
  ...
  async createFolder(userId: string, input: CreateFolderInput): Promise<IFolder> {
    // Check folder limit
  ...
  async updateFolder(userId: string, folderId: string, input: UpdateFolderInput): Promise<IFolder> {
  ...
  async deleteFolder(userId: string, folderId: string): Promise<{ movedNotesCount: number }> {
    ...
    // Move notes to root
    const moveResult = await Note.updateMany(
      { userId, folderId: new mongoose.Types.ObjectId(folderId) },
      { folderId: null }
    );
  ...
  private async deleteSubfolders(userId: string, parentId: string): Promise<void> {
    const subfolders = await Folder.find({ userId, parentId });

    for (const subfolder of subfolders) {
      // Move notes to root
      await Note.updateMany(
        { userId, folderId: subfolder._id },
        { folderId: null }
      );
